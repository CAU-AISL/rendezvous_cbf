classdef RelativeDynamics < handle
    ...
    properties
        ...
        time     % (1x1) Current simulation time [s]
    end
    ...
    methods
        % Constructor
        function obj = RelativeDynamics(J_c, m_c, initial_state, mu)
            % J_c: Inertia matrix (3x3)
            % m_c: Mass (scalar)
            % initial_state: (12x1) Initial state vector
            % mu: Gravitational parameter (optional)
            
            if nargin < 4
                mu = 398600.4418 * 1e9; % Default Earth (m^3/s^2) if not provided
            end
        ...
        function dstate = dynamics(obj, t, x, u_ctrl, u_dist, target_state)
            ...
            % 3. Attitude: dot{sigma}
            dsigma = obj.get_G_matrix(s_curr) * w_curr;
            
            % 4. Attitude: dot{omega}
        ...
        function step(obj, dt, u_ctrl, u_dist, target_state)
            ...
            k2 = obj.dynamics(t_curr + dt/2,   x_curr + dt/2*k1,  u_ctrl, u_dist, target_state);
            k3 = obj.dynamics(t_curr + dt/2,   x_curr + dt/2*k2,  u_ctrl, u_dist, target_state);
            k4 = obj.dynamics(t_curr + dt,     x_curr + dt*k3,    u_ctrl, u_dist, target_state);
            
            % Update State
            obj.state = x_curr + (dt/6.0) * (k1 + 2*k2 + 2*k3 + k4);
